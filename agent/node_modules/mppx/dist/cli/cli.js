...
const cli = Cli.create('mppx', {
    ...
    async run(c) {
        ...
        const insecure = c.options.insecure ||
            hostname === 'localhost' ||
...
const discover = Cli.create('discover', {
    ...
    async run(c) {
        ...
        if (typeof exported.openapi === 'string') {
            ...
            const mppx = exported.mppx;
            if (!mppx) {
                return c.error({
                    code: 'INVALID_MODULE',
                    message: 'Module must default-export an OpenAPI document (with `openapi` key) or an object with `mppx` (server instance) and `routes`.',
                    exitCode: 1,
                });
            }
            doc = generate(mppx, exported);
        }
    ...
    async run(c) {
        ...
        try {
            ...
            return c.error({
                ...
                message: `Invalid discovery JSON: ${error.message}`,
                exitCode: 1,
            });
        }
