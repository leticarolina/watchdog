...
import { DEFAULT_FEE, NETWORK_PASSPHRASE, SOROBAN_RPC_URLS, STELLAR_TESTNET, } from '../../constants.js';
 ...
 * ```
 */
export async function getChannelState(parameters) {
    ...
    async function simulateGetter(fnName, ...args) {
    ...
    if (!waitingPeriodVal) {
        throw new StellarMppError(`Failed to simulate refund_waiting_period on channel ${channelAddress}: missing return value`);
    }
 ...
 */
async function readCloseEffectiveAtLedger(server, channelAddress) {
...
function isEnumVariant(scVal, name) {
    try {
