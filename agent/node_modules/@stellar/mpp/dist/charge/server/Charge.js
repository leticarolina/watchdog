import { Address, FeeBumpTransaction, Keypair, Transaction, TransactionBuilder, rpc, xdr, } from '@stellar/stellar-sdk';
import { Method, Receipt } from 'mppx';
import { ALL_ZEROS, DEFAULT_DECIMALS, DEFAULT_LEDGER_CLOSE_TIME, DEFAULT_TIMEOUT, NETWORK_PASSPHRASE, SOROBAN_RPC_URLS, STELLAR_TESTNET, } from '../../constants.js';
...
import { DEFAULT_MAX_FEE_BUMP_STROOPS, DEFAULT_POLL_MAX_ATTEMPTS, DEFAULT_POLL_DELAY_MS, DEFAULT_POLL_TIMEOUT_MS, DEFAULT_SIMULATION_TIMEOUT_MS, } from '../../shared/defaults.js';
...
export function charge(parameters) {
    const { currency, decimals = DEFAULT_DECIMALS, feePayer, logger = noopLogger, maxFeeBumpStroops = DEFAULT_MAX_FEE_BUMP_STROOPS, network = STELLAR_TESTNET, pollDelayMs = DEFAULT_POLL_DELAY_MS, pollMaxAttempts = DEFAULT_POLL_MAX_ATTEMPTS, pollTimeoutMs = DEFAULT_POLL_TIMEOUT_MS, recipient, rpcUrl, simulationTimeoutMs = DEFAULT_SIMULATION_TIMEOUT_MS, store, } = parameters;
        ...
        switch (payload.type) {
            case 'hash': {
                ...
                if (challengeRequest.methodDetails?.feePayer) {
                    throw new PaymentVerificationError(`${LOG_PREFIX} Push mode (type="hash") is not allowed with feePayer=true.`);
                }
                ...
                if (store) {
                    ...
                    if (hashUsed) {
                        ...
                        throw new PaymentVerificationError(`${LOG_PREFIX} Transaction hash already used. Replay rejected.`, {
                            hash,
                        });
                    }
                }
            ...
            case 'transaction': {
                ...
                let txToSubmit = parsed;
                if (!signerKeypair && tx.source === ALL_ZEROS) {
                    logger.warn(`${LOG_PREFIX} Verification failed`, {
                        error: 'Sponsored source without feePayer',
                    });
                    throw new PaymentVerificationError(`${LOG_PREFIX} Transaction uses a sponsored source account but the server has no feePayer configuration.`, {});
                }
                ...
                if (signerKeypair && tx.source === ALL_ZEROS) {
                    ...
                    await simulateAndValidateTransfer(rebuiltTx, expectedAmount, expectedCurrency, expectedRecipient, signerKeypair.publicKey(), expectedFrom);
                    ...
                    if (expiresTimestamp && tx.timeBounds) {
                        ...
                        if (maxTime > expiresTimestamp) {
                            throw new PaymentVerificationError(`${LOG_PREFIX} Transaction timeBounds.maxTime exceeds challenge expires.`, {
                                maxTime,
                                expires: expiresTimestamp,
                            });
                        }
                    }
                    await simulateAndValidateTransfer(tx, expectedAmount, expectedCurrency, expectedRecipient, signerKeypair?.publicKey(), expectedFrom);
                }
                // ── Settlement ──────────────────────────────────────────────
                let sendResult;
                try {
                    ...
                    throw new SettlementError(`${LOG_PREFIX} Settlement failed: could not broadcast transaction.`, {
                        details: error instanceof Error ? error.message : String(error),
                    });
                }
                if (sendResult.status === 'ERROR' || sendResult.status === 'DUPLICATE') {
                    throw new SettlementError(`${LOG_PREFIX} Settlement failed: sendTransaction returned ${sendResult.status}.`, { hash: sendResult.hash, status: sendResult.status });
                }
                try {
            ...
            default:
                throw new PaymentVerificationError(`Unsupported credential type "${payload.type}".`);
        }
    }
    // ── Simulation validation ─────────────────────────────────────────────
    async function simulateAndValidateTransfer(tx, expectedAmount, expectedCurrency, expectedRecipient, serverAddress, expectedFrom) {
        let simResponse;
        try {
            ...
            if (error instanceof SimulationContractError) {
                throw new PaymentVerificationError(`${LOG_PREFIX} Pre-submission simulation failed: ${error.simulationError}`, { simulationError: error.simulationError });
            }
        ...
        if (simResponse.events && simResponse.events.length > 0) {
            validateSimulationEvents(simResponse.events, expectedAmount, expectedCurrency, expectedRecipient, serverAddress, expectedFrom);
        }
    }
    // ── Auth entry validation (sponsored path) ────────────────────────────
    async function validateAuthEntries(tx, serverPublicKey, expiresTimestamp) {
        ...
        for (let i = 0; i < ops.length; i++) {
            ...
            for (const entry of authEntries) {
                ...
                if (credentials.switch().value !==
                    ...
                    throw new PaymentVerificationError(`${LOG_PREFIX} Only address-type auth entries are permitted.`, { credentialType: credentials.switch().name });
                }
                ...
                if (entryAddress.toString() === serverAddress.toString()) {
                    throw new PaymentVerificationError(`${LOG_PREFIX} Server address must not appear in client auth entries.`, { serverAddress: serverPublicKey });
                }
                if (maxLedger !== undefined) {
                    const entryExpiration = addressCred.signatureExpirationLedger();
                    if (entryExpiration > maxLedger) {
                        throw new PaymentVerificationError(`${LOG_PREFIX} Auth entry expiration exceeds maximum allowed ledger.`, {
                            entryExpiration,
                            maxLedger,
                        });
                    }
                ...
                if (rootInvocation.subInvocations().length > 0) {
                    throw new PaymentVerificationError(`${LOG_PREFIX} Auth entries must not contain sub-invocations.`, {});
                }
            }
        }
    }
...
// ---------------------------------------------------------------------------
function verifyExactlyOneInvokeOp(tx) {
    const invokeOps = tx.operations.filter((op) => op.type === 'invokeHostFunction');
    if (invokeOps.length === 0) {
        throw new PaymentVerificationError(`${LOG_PREFIX} Transaction does not contain a Soroban invocation.`, {});
    }
    if (invokeOps.length > 1) {
        throw new PaymentVerificationError(`${LOG_PREFIX} Transaction must contain exactly one invokeHostFunction operation.`, { operationCount: invokeOps.length });
    }
}
function verifySacInvocation(tx, expected) {
    verifyFromRawOps(tx, expected);
}
function verifyFromRawOps(tx, expected) {
    let found = false;
    ...
    if (!found) {
        throw new PaymentVerificationError(`${LOG_PREFIX} Transaction does not contain a matching SAC transfer invocation.`, {
            from: expected.from,
...
function verifySacTransfer(txResult, expected, networkPassphrase) {
    if (!txResult.envelopeXdr) {
        throw new PaymentVerificationError(`${LOG_PREFIX} Transaction result is missing envelope XDR — cannot verify payment.`, {});
    }
    let envelope;
    if (typeof txResult.envelopeXdr === 'string') {
        try {
            ...
            throw new PaymentVerificationError(`${LOG_PREFIX} Could not parse transaction envelope for verification.`, {
                details: error instanceof Error ? error.message : String(error),
            });
        }
    }
...
// ---------------------------------------------------------------------------
function validateSimulationEvents(events, expectedAmount, expectedCurrency, expectedRecipient, serverAddress, expectedFrom) {
    const transferEvents = [];
    for (const event of events) {
        try {
            const contractEvent = event.event();
            // Only process contract events (type 0)
            if (contractEvent.type().value !== 0)
                continue;
            ...
            const contract = contractEvent.contractId()
                ? Address.fromScAddress(xdr.ScAddress.scAddressTypeContract(contractEvent.contractId())).toString()
                : '';
    ...
    if (transferEvents.length !== 1) {
        throw new PaymentVerificationError(`${LOG_PREFIX} Simulation produced ${transferEvents.length} transfer events; expected exactly 1.`, { count: transferEvents.length });
    }
    ...
    if (transfer.to !== expectedRecipient ||
        ...
        throw new PaymentVerificationError(`${LOG_PREFIX} Simulation transfer event does not match expected parameters.`, {
            expectedRecipient,
    ...
    if (serverAddress) {
        const serverInvolved = transferEvents.some((t) => t.from === serverAddress || t.to === serverAddress);
        if (serverInvolved) {
            throw new PaymentVerificationError(`${LOG_PREFIX} Server address must not be involved in transfer events.`, { serverAddress });
        }
    }
}
// ---------------------------------------------------------------------------
 ...
 */
function publicKeyFromDID(source) {
    if (typeof source !== 'string' || !source) {
        throw new PaymentVerificationError(`${LOG_PREFIX} Credential source is required to verify the sender address.`, {});
    }
    ...
    if (parts.length === 5 &&
        ...
        try {
            ...
            throw new PaymentVerificationError(`${LOG_PREFIX} Credential source contains an invalid Stellar public key.`, { source });
        }
        return pubKey;
    }
    throw new PaymentVerificationError(`${LOG_PREFIX} Credential source has invalid format — expected did:pkh:stellar:{network}:{pubkey}.`, { source });
}
