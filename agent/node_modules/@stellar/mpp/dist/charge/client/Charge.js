import { Account, Address, BASE_FEE, Contract, TransactionBuilder, authorizeEntry, nativeToScVal, rpc, xdr as StellarXdr, } from '@stellar/stellar-sdk';
import { Credential, Method } from 'mppx';
import { z } from 'zod/mini';
import { ALL_ZEROS, DEFAULT_DECIMALS, DEFAULT_LEDGER_CLOSE_TIME, DEFAULT_TIMEOUT, NETWORK_PASSPHRASE, SOROBAN_RPC_URLS, } from '../../constants.js';
...
import { DEFAULT_POLL_MAX_ATTEMPTS, DEFAULT_POLL_DELAY_MS, DEFAULT_POLL_TIMEOUT_MS, DEFAULT_SIMULATION_TIMEOUT_MS, } from '../../shared/defaults.js';
/**
...
export function charge(parameters) {
    const { decimals = DEFAULT_DECIMALS, keypair: keypairParam, mode: defaultMode = 'pull', onProgress, pollDelayMs = DEFAULT_POLL_DELAY_MS, pollMaxAttempts = DEFAULT_POLL_MAX_ATTEMPTS, pollTimeoutMs = DEFAULT_POLL_TIMEOUT_MS, rpcUrl, secretKey, simulationTimeoutMs: _simulationTimeoutMs = DEFAULT_SIMULATION_TIMEOUT_MS, timeout = DEFAULT_TIMEOUT, } = parameters;
    ...
    return Method.toClient(Methods.charge, {
        ...
        async createCredential({ challenge, context }) {
            ...
            if (isServerSponsored && effectiveMode === 'push') {
                throw new StellarMppError('Push mode is not supported for server-sponsored transactions. ' +
                ...
                for (const op of v1.tx().operations()) {
                    ...
                    for (let i = 0; i < authEntries.length; i++) {
                        ...
                        if (entry.credentials().switch().value ===
                            ...
                            authEntries[i] = await authorizeEntry(entry, keypair, validUntilLedger, networkPassphrase);
                        }
                    }
                }
            ...
            if (effectiveMode === 'push') {
                ...
                return Credential.serialize({
                    challenge,
                    payload: { type: 'hash', hash: result.hash },
                    source,
                });
            }
