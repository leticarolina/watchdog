...
function getRecipient() {
    ...
    if (!secretKey)
        throw new Error('RECIPIENT_SECRET_KEY_A not set');
...
───────────────────────────────────────────── */
app.get('/analysis/basic-blocked', requirePayment(PRICING.basic, 'RECIPIENT_SECRET_KEY_B'), async (_req, res) => {
...
/* ─────────────────────────────────────────────
   AGENT POOL
───────────────────────────────────────────── */
function resolvedSpent(state, windowSeconds) {
    if (!state.success)
        return 0;
    const cumulative = state.cumulativeSpent ?? 0;
    ...
    if (windowStart > 0 && nowSec - windowStart >= windowSeconds)
        return 0;
    return cumulative;
}
app.get('/agent', async (_req, res) => {
    ...
    const windowSeconds = config.success ? (config.windowSeconds ?? 86400) : 86400;
    return res.json({
        currentAgent: agentAddr,
        agentIndex: currentAgentIndex,
        totalAgents: agentPool.length,
        cumulativeSpent: resolvedSpent(state, windowSeconds),
...
app.get('/agent/state', async (_req, res) => {
    ...
    const windowSeconds = config.success ? (config.windowSeconds ?? 86400) : 86400;
    return res.json({
        spent: resolvedSpent(state, windowSeconds),
        budgetCap: config.success ? config.budgetCap : 0,
        maxSinglePayment: config.success ? config.maxSinglePayment : 0,
    });
});
app.post('/reset', async (_req, res) => {
    ...
    for (let i = 0; i < agentPool.length; i++) {
        ...
        const spent = resolvedSpent(agentState, windowSeconds);
        const windowStart = agentState.windowStart ?? 0;
        const nowSec = Math.floor(Date.now() / 1000);
    ...
    return res.json({
        ...
        agentIndex: currentAgentIndex,
        totalAgents: agentPool.length,
        cumulativeSpent: state.success ? (state.cumulativeSpent ?? 0) : 0,
        windowStart: state.success ? (state.windowStart ?? 0) : 0,
    });
});
app.listen(PORT, () => {
