...
function getRecipient(recipientEnvVar = DEFAULT_RECIPIENT_ENV_VAR) {
    const secretKey = process.env[recipientEnvVar];
    if (!secretKey)
        throw new Error(`${recipientEnvVar} not set`);
...
function getAgentSecretKey() {
    ...
    if (!secretKey)
        throw new Error('AGENT_SECRET_KEY_A not set');
    return secretKey;
}
/**
 ...
 */
async function verifyPayment(req, requiredAmount, recipientEnvVar = DEFAULT_RECIPIENT_ENV_VAR) {
    ...
    if (!credential) {
        ...
        const wwwAuthenticate = `Payment realm="watchdog", method="watchdog-request-payment", intent="charge", request="${Buffer.from(JSON.stringify(request)).toString('base64')}"`;
    ...
    const receiptHeader = Buffer.from(JSON.stringify({ reference: receipt.txHash, method: receipt.method, timestamp: receipt.timestamp })).toString('base64');
 ...
 */
export function requirePayment(requiredAmount, recipientEnvVar = DEFAULT_RECIPIENT_ENV_VAR) {
    return async function (req, res, next) {
        ...
        if ('blocked' in result) {
            console.log(`[payment] blocked agent=${getAgent()} endpoint=${req.originalUrl} reason=${result.reason} amount=${requiredAmount}`);
            return res.json({
                blocked: true,
        ...
        if (result.status === 402) {
            ...
            result.challenge.headers.forEach((value, key) => {
